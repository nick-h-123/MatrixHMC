...
struct DualValue{V<:AbstractScalarOrVec{<:AbstractFloat}, G<:AbstractArray{<:AbstractArray}}
...
function ∂H∂θ(h::Hamiltonian, θ::AbstractArray)
    res = h.∂ℓπ∂θ(θ)
...
Base.similar(z::PhasePoint{<:AbstractArray{T}}) where {T<:AbstractArray} = 
    PhasePoint(
        zeros(T, size(z.θ)...), 
        zeros(T, size(z.r)...), 
        similar(z.ℓπ), 
...
phasepoint(
    ...
    θ::AbstractArray{T},
    h::Hamiltonian
) where {T<:AbstractArray} = phasepoint(h, θ, rand(rng, h.metric))
