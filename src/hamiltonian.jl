...
struct DualValue{V<:AbstractScalarOrVec{<:AbstractFloat}, G<:AbstractVecOrMat{<:AbstractFloat}}
    ...
    function DualValue(value::V, gradient::G) where {V, G}
        ...
        """if value isa AbstractFloat
            ...
            @assert gradient isa AbstractVector "`typeof(gradient)`: 
        else
            # If `value` is a vector, `gradient` is a matrix
            @assert gradient isa AbstractMatrix "`typeof(gradient)`: 
        end"""
...
function ∂H∂r(h::Hamiltonian{<:DiagEuclideanMatrixMetric}, r::AbstractVecOrMat) 
    res = copy(r)
    Minv = h.metric.M⁻¹
    res[:,1] .*= Minv
    res[:,2] .*= Minv
...
function neg_energy(h::Hamiltonian{<:DiagEuclideanMatrixMetric}, r::T, θ::T) where {T<:AbstractVecOrMat} 
    rReal = r[:,1]
    rImag = r[:,2]
    r_arr = r[:,1] + im*r[:,2]
    return -sum(abs2.(r_arr) .* h.metric.M⁻¹) / 2
end
