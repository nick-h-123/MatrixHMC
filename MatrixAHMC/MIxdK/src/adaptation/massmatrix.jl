...
mutable struct WelfordVar{T<:AbstractFloat, E<:AbstractVecOrMat{T}, E2<:AbstractVecOrMat{T}} <: DiagMatrixEstimator{T}
    n     :: Int
    n_min :: Int
    μ     :: E
    M     :: E
    δ     :: E     # cache for diff
    var   :: E2    # cache for variance
    function WelfordVar(n::Int, n_min::Int, μ::E, M::E, δ::E, var::E2) where {E, E2}
        return new{eltype(E), E, E2}(n, n_min, μ, M, δ, var)
    end
end
...
function get_estimation(wv::WelfordVar{T}) where {T<:AbstractFloat}
    ...
    n, ϵ = T(n), T(1e-3)
    M_arr = reshape(Statistics.mean(M, dims=2), size(M)[1])
    return n / ((n + 5) * (n - 1)) * M_arr .+ ϵ * (5 / (n + 5))
end
